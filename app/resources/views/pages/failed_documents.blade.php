...
@section('page_title', 'Failed documents')
...
<div class="row" ng-controller="failed_docs_controller">
  <!-- search filter -->
      ...
      <table class="table table-hover table-striped">
        ...
        <tbody style="font-size:13px;">
          <tr ng-repeat="data in filtered = (list | filter:search ) | startFrom:(currentPage-1)*entryLimit | limitTo:entryLimit track by $index" >
            ...
            <td>
              <span>
                <span ng-if="data.process_status=='ocred_final_failed'">
                  <!-- document edit -->
...
app.filter('ocred_status', function(){
      ...
      return function(data)
      {
          if(data=='password_protected'){
              data = "<b class='ocr_failed'>"+"Password protected"+"</b>";
          }
          else if(data=='not_pdf')
          {
              data = '<b class="ocr_failed">'+'Invalid file type'+'</b>';
          }else{
              data = '<b class="ocr_failed">'+'Unable to OCR pdf'+'</b>'; 
          }
...
app.controller('failed_docs_controller', function($scope, $http, $timeout) {

$scope.preloader        =   true;
$scope.doc_table        =   false;
$scope.defaultOptionVal =   'none';

$scope.get_failed_documents = function(){

   $http.get('/ocr_failed_documents').success(function(data){
        $scope.list = data;
